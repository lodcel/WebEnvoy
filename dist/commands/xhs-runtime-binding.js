...
const isXhsReadCommand = (command) => command === "xhs.search" || command === "xhs.detail" || command === "xhs.user_home";
const toRuntimeExecutionMode = (requestedExecutionMode) => requestedExecutionMode === "live_read_limited" ||
    requestedExecutionMode === "live_read_high_risk"
    ? "read"
    : requestedExecutionMode === "dry_run" || requestedExecutionMode === "recon"
        ? "diagnose"
        : "unknown";
const buildTargetCandidateRef = (runId, targetTabId) => targetTabId === null ? null : `target-tab-ref:${runId}:${targetTabId}`;
const resolveTargetBindingState = (input) => {
    if (!input.targetCandidateRef) {
        return "unbound";
    }
    if (input.targetDomain !== "www.xiaohongshu.com" ||
...
export const buildXhsDriverRuntimeBindingForContract = (input) => {
    ...
    const transitionEvidence = state === "candidate_found" && targetCandidateRef
        ? [
            {
...
export const toXhsDriverRuntimeBindingSummaryFields = (boundary) => {
    if (!boundary) {
        return {};
    }
    return {
