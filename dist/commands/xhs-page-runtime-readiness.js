...
const isRuntimeReadinessRequired = (mode) => mode === "recon" || mode === "live_read_limited" || mode === "live_read_high_risk";
const isPageReadinessRequired = (mode) => mode === "recon" || mode === "live_read_limited" || mode === "live_read_high_risk";
const asObject = (value) => value && typeof value === "object" && !Array.isArray(value) ? value : null;
const asString = (value) => typeof value === "string" && value.trim().length > 0 ? value.trim() : null;
const asBoolean = (value) => typeof value === "boolean" ? value : null;
const asStringArray = (value) => Array.isArray(value) ? value.filter((item) => typeof item === "string") : [];
const dedupe = (items) => items.filter((item, index, list) => list.indexOf(item) === index);
const statusIsPassing = (status) => status === "ready" || status === "not_required";
...
const buildRuntimeReadiness = (input) => {
    ...
    const runtimeReadiness = asString(runtimeStatus?.runtimeReadiness ?? runtimeStatus?.runtime_readiness);
    const executionSurface = asString(runtimeStatus?.executionSurface ?? runtimeStatus?.execution_surface);
    ...
    const status = runtimeReadiness === "pending" || runtimeReadiness === "recoverable" || runtimeReadiness === "unknown"
        ? runtimeReadiness
        : blockingReasons.length === 0
            ? "ready"
            : "blocked";
...
const providerAdmissionRefsMatch = (input) => {
    const refs = asStringArray(input.providerAdmissionResult?.provider_requirement_refs ??
...
const buildProviderAdmissionReadiness = (input) => {
    ...
    const admissionDecision = asString(providerAdmissionResult?.decision ?? providerAdmissionResult?.admission_decision);
    const providerRefsMatch = providerRequirementRefs.length > 0 &&
...
const toOverallReadiness = (statuses) => {
...
export const toXhsPageRuntimeReadinessSummaryFields = (contract) => {
    if (!contract) {
        return {};
    }
    return {
