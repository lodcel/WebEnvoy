...
const parseRequiredString = (source, key, reason, abilityId = "l2.first_usable") => {
...
const parseOptionalString = (source, key, reason, abilityId = "l2.first_usable") => {
    if (!hasOwn(source, key) || source[key] === undefined) {
        return undefined;
    }
    return parseRequiredString(source, key, reason, abilityId);
};
const parseRiskGateContext = (value) => {
...
const parseAllowedActions = (value) => {
    ...
    for (const item of value) {
        if (typeof item !== "string" || !ALLOWED_ACTIONS.has(item)) {
            throw invalidL2Input("ALLOWED_ACTIONS_INVALID");
        }
        const action = item;
        if (seen.has(action)) {
...
export const parseL2FirstUsableRequestForContract = (value, runtime) => {
    ...
    const safetyClass = parseRequiredString(object, "interaction_safety_class", "INTERACTION_SAFETY_CLASS_INVALID");
...
export const buildL2RiskGateBlockedResultForContract = () => ({
    success: false,
    failure_class: "risk_gate_blocked"
});
export const buildL2RequiresL1FallbackResultForContract = (fallbackReason, input) => {
    if (!FALLBACK_REASONS.has(fallbackReason)) {
        throw invalidL2Input("L1_FALLBACK_REASON_INVALID");
    }
    const recommendedStrategy = input?.recommendedStrategy ?? (fallbackReason === "target_not_located" ? "visual_reacquire" :
...
const parseTraceStep = (value, abilityId) => {
    const object = asObject(value);
    if (!object) {
...
const parseInteractionTraceStep = (value, abilityId) => {
    const object = asObject(value);
    if (!object) {
    ...
    const semantics = parseRequiredString(object, "interaction_semantics", "INTERACTION_SEMANTICS_INVALID", abilityId);
    if (action === "click") {
...
const parseCandidateShellSeed = (value, request) => {
    const object = asObject(value);
    if (!object) {
    ...
    if (!Array.isArray(executionLayerSupport) ||
    ...
    return {
        ...
        capture_origin: "l2_first_usable_sample",
        candidate_status: "draft_candidate",
        contract_registry_seed: registryInput
    };
};
export const parseL2FirstUsableResultForContract = (value, request) => {
    const source = asObject(value);
    ...
    if (object.success === false) {
        ...
        const recommendedStrategy = parseRequiredString(payload, "recommended_strategy", "L1_RECOMMENDED_STRATEGY_INVALID");
        if (fallbackGoal !== "read" ||
            !FALLBACK_REASONS.has(fallbackReason) ||
            !RECOMMENDED_STRATEGIES.has(recommendedStrategy)) {
    ...
    const interactionTrace = object.interaction_trace.map((step) => parseInteractionTraceStep(step, "l2.first_usable"));
...
export const materializeCandidateAbilityFromL2SeedForContract = (seed) => {
    const descriptor = parseCandidateAbilityDescriptorForContract({
...
export const buildAbilityValidationSeedForL2Result = (request, result) => {
    const materialized = materializeCandidateAbilityFromL2SeedForContract(result.candidate_shell_seed);
    return {
        ...materialized,
