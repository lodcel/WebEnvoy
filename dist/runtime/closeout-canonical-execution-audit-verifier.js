...
const auditRequestRefMatchesConsumedActionRef = (executionAudit) => {
    const consumedInputs = asObject(executionAudit.consumed_inputs);
    return (asNonEmptyString(executionAudit.request_ref) ===
...
export const verifyCloseoutCanonicalExecutionAudit = (input) => {
    ...
    if (input.success) {
        ...
        if (isCanonicalRequestAdmissionResult(successRequestAdmissionResult) &&
            ...
            blockers.push(blocker("success_consumed_inputs_mismatch", "canonical_consistency", "success.summary.execution_audit.consumed_inputs.action_request_ref", "success execution_audit action_request_ref must match execution_audit request_ref"));
        }
        if (isCanonicalRequestAdmissionResult(successRequestAdmissionResult) &&
            isCanonicalExecutionAudit(successExecutionAudit) &&
    ...
    if (input.failure) {
        ...
        if (isCanonicalRequestAdmissionResult(failureRequestAdmission) &&
            ...
            blockers.push(blocker("failure_consumed_inputs_mismatch", "canonical_consistency", `failure.${failureDetails?.path ?? "details"}.execution_audit.consumed_inputs.action_request_ref`, "failure execution_audit action_request_ref must match execution_audit request_ref"));
        }
        if (isCanonicalRequestAdmissionResult(failureRequestAdmission) &&
            isCanonicalExecutionAudit(failureDetailsExecutionAudit) &&
