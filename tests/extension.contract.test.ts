...
const createProviderAwareDetailReadyReadPathOptions = (runId: string) =>
  ({
...
const createProviderAwareUserHomeReadyReadPathOptions = (runId: string) =>
  ({
    xhs_driver_provider_requirements: {
...
const withProviderAwareDetailReadOptions = <T>(input: T): T => {
  const commandInput = asPlainRecord(input);
  const options = asPlainRecord(commandInput?.options);
  if (
    !commandInput ||
    !options ||
    commandInput.abilityId !== "xhs.note.detail.v1" ||
    commandInput.abilityAction !== "read" ||
    options.requested_execution_mode !== "live_read_high_risk"
  ) {
    return input;
  ...
  const runId =
    typeof executionContext?.runId === "string"
      ? executionContext.runId
      : "run-contract-detail-live-001";
  return {
    ...commandInput,
    options: {
      ...createProviderAwareDetailReadyReadPathOptions(runId),
      ...options
    }
  } as T;
};

const withProviderAwareUserHomeReadOptions = <T>(input: T): T => {
  const commandInput = asPlainRecord(input);
  const options = asPlainRecord(commandInput?.options);
  if (
    !commandInput ||
    !options ||
    commandInput.abilityId !== "xhs.user.home.v1" ||
    commandInput.abilityAction !== "read" ||
    options.requested_execution_mode !== "live_read_high_risk"
  ) {
    return input;
  ...
  const runId =
    typeof executionContext?.runId === "string"
      ? executionContext.runId
      : "run-contract-user-home-live-001";
  return {
    ...commandInput,
    options: {
      ...createProviderAwareUserHomeReadyReadPathOptions(runId),
      ...options
    }
  } as T;
};

const withProviderAwareBundledReadOptions = <T>(input: T): T =>
  withProviderAwareUserHomeReadOptions(
    withProviderAwareDetailReadOptions(withProviderAwareSearchReadOptions(input))
  );
...
const executeBundledXhsCommand = async (
  ...
  return executeCommand?.(
    withProviderAwareBundledReadOptions(input.commandInput),
    {
