...
export const isXhsControlledPublishIdentityDiagnosticRequestUrl = (url, method) => {
...
export const isXhsControlledPublishIdentityIgnoredDiagnosticRequestUrl = (url, method) => {
    if (!/^(GET|POST|PUT|PATCH)$/iu.test(method)) {
        return false;
    }
    try {
...
export const extractXhsControlledPublishResultIdentityCapture = (input) => {
    if (input.status < 200 ||
        input.status >= 300 ||
...
export const applyXhsControlledPublishResultIdentityCapture = (result, capture) => {
    if (!capture) {
        return result;
    }
    const evidence = result.live_write_evidence;
