...
type ActiveApiFetchFallbackGateResult =
  | {
      ...
      signed_continuity_binding: JsonRecord;
    }
  | {
...
const PAGE_STATE_FALLBACK_HARD_STOP_REASONS = new Set<string>([
  "XHS_LOGIN_REQUIRED",
  "SESSION_EXPIRED",
  "ACCOUNT_ABNORMAL",
  "XHS_ACCOUNT_RISK_PAGE",
...
const canUsePageStateFallbackForReason = (
  spec: XhsReadCommandSpec,
  params: XhsDetailParams | XhsUserHomeParams,
  pageStateRoot: unknown,
  reason: string
): boolean =>
  !PAGE_STATE_FALLBACK_HARD_STOP_REASONS.has(reason) &&
...
const isRedactedCapturedHeaderValue = (value: string): boolean =>
...
const isSyntheticActiveFetchBootstrapArtifact = (value: unknown): boolean => {
  const record = asRecord(value);
  return (
    asString(record?.transport) === "synthetic_active_fetch_bootstrap" ||
...
const extractXhsUserHomePageUserId = (value: string | null): string | null => {
  if (!value || isSecurityRedirectUrl(value)) {
    return null;
  }
  try {
...
const isSameXhsReadPageBinding = (input: {
  spec: XhsReadCommandSpec;
  templatePageUrl: string | null;
  currentPageUrl: string | null;
  signedContinuity: XhsSignedContinuity;
}): boolean =>
  input.spec.pageKind === "user_home"
...
const isSignedContinuityBoundToUserHome = (
  params: XhsUserHomeParams,
  continuity?: XhsSignedContinuity | null
): boolean =>
  continuity?.token_presence === "present" &&
...
const isEmptyUserPostedSuccessForSignedUserHome = (
  body: unknown,
  params: XhsUserHomeParams,
  continuity?: XhsSignedContinuity | null
): boolean => {
...
const createExplicitRequestContextResult = (input: {
  ...
  artifact: unknown;
  env: XhsSearchEnvironment;
}): ReadRequestContextLookupResult | null => {
...
const normalizeBindingRefSegment = (value: string | null): string =>
  encodeURIComponent(value ?? "unknown").replace(/%/g, "_");

const stripXhsUrlQuery = (value: string | null): string | null => {
  if (!value) {
    return null;
  }
  try {
...
const summarizeSignedContinuityForBinding = (
  continuity: XhsSignedContinuity
): JsonRecord => ({
  source_route: continuity.source_route,
  xsec_source: continuity.xsec_source,
  token_presence: continuity.token_presence,
  credential_value_redaction_state:
    continuity.token_presence === "present" ? "redacted" : "not_applicable",
...
const buildSignedContinuityBindingEvidence = (input: {
  ...
  binding: JsonRecord;
  pageUrl: string;
}): { evidence: JsonRecord; blockers: string[] } => {
  ...
  blockers.push(
    ...collectTargetBindingEvidenceBlockers(
      ...
      transitionEvidence,
      input.executionInput.executionContext.runId
    ).map((reason) => `target_binding:${reason}`),
    ...collectRuntimeBindingBlockers(
      input.spec,
      runtimeBindingRef,
      runtimeBinding,
      input.executionInput.executionContext.runId
    ).map((reason) => `runtime_binding:${reason}`)
  );
  ...
  return {
    ...
    evidence: {
      ...
      provenance: {
        ...
        page_context_namespace: input.templateEvidence.page_context_namespace,
        shape_key: input.templateEvidence.shape_key,
        target_binding_transition_refs: asStringArray(
          asRecord(targetBindingSnapshot?.evidence_refs)?.transition_refs
        )
      },
...
const buildProviderAwareReadPathSummaryFields = (options: XhsSearchOptions): JsonRecord => {
  const providerRequirementRefs = asStringArray(options.provider_requirement_refs);
  const targetBindingTransitionEvidence = asRecordArray(
    options.target_binding_transition_evidence
  );
  const downstreamSliceRefs = asStringArray(options.downstream_slice_refs);
  const nonProofs = asStringArray(options.non_proofs);
  const pageRuntimeReadinessBlockingReasons = asStringArray(
    options.page_runtime_readiness_blocking_reasons
  );
  ...
  return {
    ...
    ...(asRecord(options.xhs_runtime_binding)
      ? { xhs_runtime_binding: asRecord(options.xhs_runtime_binding) as JsonRecord }
      : {}),
    ...(asRecord(options.target_binding_snapshot)
      ? { target_binding_snapshot: asRecord(options.target_binding_snapshot) as JsonRecord }
      : {}),
    ...
    ...(asRecord(options.xhs_page_runtime_readiness)
      ? { xhs_page_runtime_readiness: asRecord(options.xhs_page_runtime_readiness) as JsonRecord }
      : {}),
...
const collectTargetBindingEvidenceBlockers = (
  ...
  if (!freshnessScope) {
    ...
    if (
      freshnessScope === "historical_background" ||
...
const resolveBlockedNextRiskState = (
  current: ReturnType<typeof createAuditRecord>["risk_state"]
): ReturnType<typeof createAuditRecord>["risk_state"] => {
  if (current === "allowed") {
    return "limited";
  }
  if (current === "limited") {
...
const isProviderAwareLiveReadGate = (gate: ReturnType<typeof resolveGate>): boolean =>
...
const createPassiveApiCaptureSuccess = (
  ...
  if (
    ...
    !responseContainsRequestedTarget(
      spec,
      input.params,
      requestContextResult.responseBody,
...
const responseContainsRequestedTarget = (
  ...
  ) || isEmptyUserPostedSuccessForSignedUserHome(body, params as XhsUserHomeParams, continuity);
};
...
const parseDetailLikeNoteIdFromHref = (href: string): string | null => {
  try {
...
const parseUserProfileIdFromHref = (href: string): string | null => {
  try {
    const parsed = new URL(href);
...
const normalizeXhsDetailTitle = (title: string): string | null => {
...
const normalizeXhsUserHomeTitle = (title: string): string | null => {
  const normalized = title
    .replace(/\s+-\s+小红书\s*$/u, "")
    .replace(/\s*\|\s*小红书\s*$/u, "")
    .trim();
...
const hasBlockingPageSurface = (bodyText: string): boolean => {
...
const createDetailDomPageStateRoot = (
  params: XhsDetailParams,
  env: XhsSearchEnvironment
): JsonRecord | null => {
...
const createUserHomeDomPageStateRoot = (
  params: XhsUserHomeParams,
  env: XhsSearchEnvironment
): JsonRecord | null => {
  const href = env.getLocationHref();
...
const executeXhsRead = async (
  ...
  const resolvePageStateRoot = async (): Promise<JsonRecord | null> => {
    ...
    return spec.command === "xhs.detail"
      ? createDetailDomPageStateRoot(input.params as XhsDetailParams, env)
      : createUserHomeDomPageStateRoot(input.params as XhsUserHomeParams, env);
  };
  ...
  let requestContextResult =
    createExplicitRequestContextResult({
      spec,
      expectedShape,
      artifact: input.options.explicit_request_context_artifact,
      env
    }) ??
    (await readCapturedReadContextWithRetry(
      spec,
      expectedShape,
      env,
  ...
  if (!responseContainsRequestedTarget(
    spec,
    input.params,
    response.body,
    requestContextResult.signedContinuity
