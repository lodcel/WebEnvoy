...
type SearchRequestContextHit = Extract<RequestContextState, { status: "hit" }>;
...
const withPassiveApiCaptureEvidenceDiagnostic = (routeEvidence: JsonRecord): JsonRecord => ({
...
const buildSearchDomPageStateFallbackEvidence = (input: {
  extraction: SearchDomExtraction;
  ...
  try {
    return asRecord(
      await env.performSearchPassiveAction({
        ...
        timeoutMs: Math.min(
          typeof input.options.timeout_ms === "number" &&
...
const resolveRequestContextState = async (
  ...
  const maxAttempts =
    requestInput.failFastOnMiss === true
      ? 1
      : resolveRequestContextWaitMaxAttempts(
...
const resolveDebuggerNetworkRequestContextState = (input: {
  passiveActionEvidence: JsonRecord | null;
  query: string;
  pageContextNamespace: string;
  shapeKey: string;
  now: number;
  ...
  return {
    ...
    template: {
      request: {
        url,
        headers: (asRecord(request?.headers) as Record<string, string> | null) ?? {},
...
const describePassiveActionEvidenceForDiagnosis = (
  passiveActionEvidence: JsonRecord | null
): string[] => {
  ...
  const summarizeValue = (key: string): string | null => {
...
export const executeXhsSearch = async (
  ...
  if (
    input.options.issue_scope === "issue_835" &&
    input.options.controlled_live_write === true &&
    input.options.target_page === "creator_publish_tab" &&
    gate.consumer_gate_result.effective_execution_mode === "live_write"
  ) {
    const publishVisibilityScope =
      input.options.publish_visibility_scope === "private_or_self_visible" ||
      ...
      input.options.publish_visibility_scope === "public_visible"
        ...
        : null;
    const cleanupPolicyRef =
      typeof input.options.cleanup_policy_ref === "string" &&
      input.options.cleanup_policy_ref.trim().length > 0
        ? input.options.cleanup_policy_ref.trim()
        : null;
    const liveWriteAttemptId =
      typeof input.params.live_write_attempt_id === "string" &&
    ...
    const sourceMediaRef =
      typeof input.params.source_media_ref === "string" ? input.params.source_media_ref : "";
    const sourceMediaDigest =
      typeof input.params.source_media_digest === "string" ? input.params.source_media_digest : "";
    const sourceMediaKind =
      input.params.source_media_kind === "video" || input.params.source_media_kind === "mixed"
    ...
    if (!publishVisibilityScope || !cleanupPolicyRef) {
      return withExecutionAuditInFailurePayload(
        createFailure(
          "ERR_EXECUTION_FAILED",
          "FR-0032 controlled live write policy missing",
          {
            ability_id: input.abilityId,
            stage: "execution",
    ...
    const controlledLiveWriteInput: Parameters<NonNullable<XhsSearchEnvironment["performControlledLiveWrite"]>>[0] = {
      ...
      background_upload_capture_continuation:
        input.params.background_upload_capture_continuation === true
    };
  ...
  if (
    input.options.__request_context_provenance_confirmed !== true &&
    !(await confirmCurrentRequestContextProvenance())
  ) {
    return createProvenanceUnconfirmedFailure();
  }
  const closeoutPassiveExactHitOnly =
    input.options.closeout_evidence_evaluation === true ||
    input.options.closeout_audit_required === true;
  let passiveActionStartedAt: number | null = null;
  ...
  const rememberCloseoutRequestContextHit = (state: RequestContextState): void => {
    ...
    if (
      closeoutRequestContextHits.some(
        (candidate) =>
          [
      ...
      state.status !== "hit"
        ? resolveDebuggerNetworkRequestContextState({
            passiveActionEvidence: passiveActionEvidence
              ? (passiveActionEvidence as JsonRecord)
              : null,
            query: input.params.query,
  ...
  if (requestContextState.status !== "hit") {
    requestContextState =
      resolveDebuggerNetworkRequestContextState({
        passiveActionEvidence: passiveActionEvidence ? (passiveActionEvidence as JsonRecord) : null,
        query: input.params.query,
    ...
    return withExecutionAuditInFailurePayload(
      createFailure(
        ...
        createDiagnosis({
          ...
          category: isBackendRejectedSource ? "request_failed" : "page_changed",
          evidence: [
            ...describePassiveActionEvidenceForDiagnosis(
              passiveActionEvidence ? (passiveActionEvidence as JsonRecord) : null
            ),
            `request_context_reason=${requestContextState.failureReason}`,
  ...
  const targetTabId =
    typeof input.options.actual_target_tab_id === "number"
      ...
      : typeof input.options.target_tab_id === "number"
        ...
        : gate.consumer_gate_result.target_tab_id;
  const closeoutRoundHits =
    closeoutPassiveExactHitOnly && closeoutRequestContextHits.length > 0
  ...
  const buildCloseoutEvidenceRound = (state: SearchRequestContextHit): JsonRecord => {
  ...
  const closeoutEvidenceExpected: JsonRecord = {
    latest_head_sha: routeEvidence.head_sha,
    run_id: routeEvidence.run_id,
    artifact_identity: routeEvidence.artifact_identity,
