...
closeout issue 的输入应是已经具备 readiness/admission 前置和明确关闭条件的执行现场；它不是实现探索、需求发现或反复 live probing 的入口。

- 进入 closeout 前，先声明 `issue_type`、关闭条件、readiness/admission 状态和允许执行的最小验证范围
- high-risk live / real-browser / account-touching closeout 必须先有 readiness 或 admission 证据；没有证据时只允许产出 blocked/preflight 结论
- readiness matrix 至少覆盖：runtime 是否 ready、profile / lock / extension / native messaging 是否一致、account safety 是否 clear、anti-detection baseline 是否 ready、gate/admission/audit 是否齐、route evidence evaluator 是否可判定、required headers verifier 是否存在、stop / cleanup 是否可证明
- live 验证必须阶梯推进：`static tests -> runtime.status/audit -> dry_run -> recon -> 单路线 limited live -> route evidence evaluator -> full bundle`；任一步失败即停，不继续跑后续路线
- 一轮 live 只回答一个问题；不得用同一次 live run 同时验证 search/detail/user_home、headers、恢复、风控和 cleanup
- live 失败后必须停在失败检查点，并把 blocker 拆给对应实现、准入、账号安全、证据契约或环境归属；不得继续执行剩余 bundle 扩大风险或污染证据链
- fallback 证据只证明退路或产品价值，不等于 formal closeout 证据；DOM/state extraction 与正式 API closeout、required-header matrix、canonical audit 等证据必须分栏记录
- closeout PR 必须使用当前 PR latest head 或 current latest main 的 fresh evidence；历史 artifact、旧 head、旧 run 和 same-head 历史产物只能作为背景，不得作为关闭依据
- PR 元数据必须写明 closeout 证据、fallback 限制、readiness/admission 状态和 blocker 拆分处理；缺失时保持 `Refs #...` 与非 `merge-ready`
...
- 若 Spike 后续进入 closeout-only issue，`admission_ready` 仍只是进入 closeout 的前置，不是关闭证据；关闭证据必须在 closeout PR 的 latest-head fresh evidence 中单独给出
...
例外：已明确进入 Loom Work Item / recovery flow 的事项，可以把结构化恢复载体写入 `.loom/progress/<issue>.md` 和 `.loom/status/current.md`。这些载体只记录执行恢复所需的 checkpoint、阻断、验证摘要、下一步和 locator；不得写入第二套 backlog、sprint、project status 或 issue state。GitHub Issues / Projects 仍是 progress / backlog truth；本机绝对 worktree 路径应留在运行态 host binding 或本地 handoff，不写入仓库事实链。
