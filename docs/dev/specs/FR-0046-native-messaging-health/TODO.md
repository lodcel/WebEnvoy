...
- [ ] 确认 required/requested `native_messaging` 只能由全部 required provider-level checks `status=pass` 且无 blocking/fatal evidence 满足，`not_applicable` 不得满足 `official-chrome.persistent` admission。
...
- [ ] 后续 tests 覆盖 missing/not_applicable/unknown/fail required checks、stub/fake host evidence、redaction invalid 与 capability blocking。
