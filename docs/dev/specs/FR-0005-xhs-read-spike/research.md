...
因此，2026-04-16 这轮 blocker refresh 不在本文件逐项展开；当时 formal 结论也未因此发生变化，FR-0005 继续保持 `No-Go/paused`。2026-05-16 PR `#682` 已补齐 `primary api multi-round` 子条件；随后 5.8 的 current-main 复核补齐 required-header minimal matrix 后，FR-0005 closeout 进入完成态。
...
- 5.8 已在 current main `a289576e990dba5caecf0d08f8a1d8db1187e0f9` 上补齐剩余 required-header minimal matrix 与 stop/process evidence，因此 `#445` 可在对应 closeout PR 受控合并后关闭。
...
- PR `#683` 的正确语义已从 partial closeout 回写升级为 full closeout 回写；该升级只基于 5.8 的 current-main evidence，不复用本节失败 rerun 作为 success gate。

### 5.8 2026-05-16 issue #445 current-main full closeout 记录

本节记录 PR `#689` 合入后的 current main fresh closeout rerun。该轮从 `main` head `a289576e990dba5caecf0d08f8a1d8db1187e0f9` 启动，不修改 PR `#682`，不使用 PR `#683` docs-only 失败 rerun 作为成功证据。

执行身份与 artifact：

- current main head：`a289576e990dba5caecf0d08f8a1d8db1187e0f9`
- run_id：`issue445-current-main-a289576-20260516T1833Z`
- profile：`xhs_001`
- execution_surface：`real_browser`
- browser_channel：`chrome`
- native host install proof：`.webenvoy/issue445-runtime-install-a289576-20260516T1833Z.json`
- search evidence：`.webenvoy/issue445-search-a289576-20260516T1834Z-r1-live.json`
- detail evidence：`.webenvoy/issue445-open-detail-a289576-20260516T1834Z-r1.json`、`.webenvoy/issue445-open-detail-a289576-20260516T1841Z-r2.json`、`.webenvoy/issue445-detail-a289576-20260516T1842Z-r1-live.json`
- user_home evidence：`.webenvoy/issue445-capture-userhome-a289576-20260516T1845Z-r1.json`、`.webenvoy/issue445-capture-userhome-a289576-20260516T1845Z-r2.json`、`.webenvoy/issue445-userhome-a289576-20260516T1846Z-r1-live.json`
- required-header matrix：`.webenvoy/issue445-required-headers-matrix-a289576-20260516T1847Z-input-result.json`
- minimality proof artifact：`.webenvoy/issue445-required-headers-matrix-a289576-20260516T1847Z-proof.json`
- shutdown evidence：`.webenvoy/issue445-stop-a289576-20260516T1848Z.json`、`.webenvoy/issue445-status-post-stop-a289576-20260516T1848Z.json`、`.webenvoy/issue445-post-stop-processes-a289576-20260516T1851Z.txt`
...
| --- | --- | --- | --- | --- |
| `xhs.search` | `accept`, `content-type`, `x-s`, `x-s-common`, `x-t` | 是 | `.webenvoy/issue445-required-headers-matrix-a289576-20260516T1847Z-proof.json#xhs.search/*` | `PASS` |
| `xhs.detail` | `accept`, `content-type`, `x-s`, `x-s-common`, `x-t` | 是 | `.webenvoy/issue445-required-headers-matrix-a289576-20260516T1847Z-proof.json#xhs.detail/*` | `PASS` |
| `xhs.user_home` | `accept`, `x-s`, `x-s-common`, `x-t` | 是 | `.webenvoy/issue445-required-headers-matrix-a289576-20260516T1847Z-proof.json#xhs.user_home/*` | `PASS` |
...
- post-stop process scan `.webenvoy/issue445-post-stop-processes-a289576-20260516T1851Z.txt` 为 0 行

最终 formal 结论：

- `search/detail/user_home` 三条命令均已在 current main 上达到 `route_role=primary + path_kind=api + evidence_status=success + reproduced_multi_round`。
...
- FR-0005 / issue `#445` 的 full closeout 条件已由 current-main evidence 满足；待本 PR 通过 guardian、GitHub checks 并受控合并后，可用本节证据链关闭 `#445`。
