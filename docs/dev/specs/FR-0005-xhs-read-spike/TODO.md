...
- [x] 2026-04-11 固定样本头的 formal FR 文档收口已完成；当时正式功能停点为：`search/detail/user_home` 仍缺 `route_role=primary + path_kind=api + evidence_status=success + reproduced_multi_round`，其中 `search` 还需补齐 required headers 最小必要集矩阵；2026-05-16 PR `#682` 已补齐 `primary api multi-round` 子条件，但 full closeout 仍受 required-header minimal matrix 阻断
...
- [x] 为 `search/detail/user_home` 各端点补齐 PR `#682` partial closeout 的 observed request context 记录（以 `real_browser` passive API capture 的 browser-owned request context 为准；不关闭手工 required-header minimal matrix，也不把手工 header reconstruction 或字段生命周期升级为 `admission_ready`）
- [ ] 未登录 / 会话过期 / 风控拦截在 WebEnvoy 诊断壳中的最终映射

## 后续衔接

- [x] 2026-04-06 中午已形成本轮 Go/No-Go 历史结论：`No-Go/paused`
- [x] 上述 `No-Go/paused` 继续保留为带日期的历史 closeout；2026-05-16 PR `#682` 已补齐 `primary api multi-round` 子条件，但 full closeout 仍受 required-header minimal matrix 阻断
...
- [x] 上述 blocker refresh 不改写 FR-0005 formal closeout bar；2026-05-16 已满足 `primary + api + success + reproduced_multi_round` 子条件，但手工 required headers 最小必要集矩阵仍留待后续 L3 实现 FR 单独关闭
- [x] 2026-05-16 已完成 PR `#682` live evidence 复核并更新正式状态：历史 `No-Go/paused` 章节保留为 dated fact，当前 `primary api multi-round` 子条件已完成，但 `#445` full closeout 仍 blocked
...
- [ ] 完成 required-header minimal matrix 后，再决定是否关闭 `#445` / 进入后续实现 FR
