...
- `provider_requirement_ref`: commit-scope provider requirement disposition from a future commit provider owner; #1179 admission refs are not valid for this field.
...
- `provider_requirement_ref`: accepted commit-scope provider requirement disposition from a provider owner that is not #1179 `xhs.creator_publish.admit`.
...
- #1179 `xhs.creator_publish.admit` supports only `write_admit`; it is admission-only context and cannot be promoted, upgraded or reconsumed as `provider_requirement_ref` for `live_write_commit`.
...
4. `research.md` records the consumed facts from #1174-#1179 and confirms #1179 remains admission-only `write_admit` background, not release-ready provider evidence or commit unlock.
