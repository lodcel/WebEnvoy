...
interface ProviderDoctorInputContractRef {
  ...
  expected_binary_source: ProviderDoctorExpectedBinarySource
  capability_ids_requested: string[]
}

interface ProviderDoctorExpectedBinarySource {
  source_id: string
...
- `expected_binary_source` 是 FR-0038 doctor/admission input 自己冻结的 binary source，不来自 `FR-0033.provider_identity`；缺失、locator 不可解析、source kind 未知或 access 不满足 `expected_access` 时，provider-level `binary` required check 必须 fail-closed。
...
| Required source | Required doctor category |
|---|---|
| `ProviderDoctorInputContractRef.expected_binary_source` | `binary` |
...
```json
{
  ...
  "input_contract_ref": {
    ...
    "expected_binary_source": {
      "source_id": "official-chrome-stable-binary",
      "source_kind": "browser_executable",
      "locator_ref": "doctor-input://official-chrome-stable/redacted-binary",
      "locator_sensitivity": "sensitive",
      "expected_access": "executable"
    },
  ...
  "checks": [
    {
      ...
      "diagnostics": {
        ...
        "observed": "source_id=official-chrome-stable-binary; access=executable",
        "expected": "source_id=official-chrome-stable-binary; expected_access=executable"
      },
      "evidence_refs": [
        {
          "kind": "local_file_ref",
