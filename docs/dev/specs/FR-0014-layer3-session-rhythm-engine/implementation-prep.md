...
本文档用于把 `FR-0014 / #237 / #742` 收口到 `implementation-ready` 输入，不修改正式 spec 语义，不混入实现代码。
...
`#742` 只负责冻结完整 Layer 3 session rhythm 契约与状态机输入。后续代码实现应拆到下游 work item：

- `#743`：profile 级长期 session 历史与冷却预算
- `#744`：read/write/recovery admission 统一节律决策
- `#745`：异常与 risk 状态恢复策略
- `#746`：多运行验证基线与回归门禁

因此，`#742` 的 PR 应使用 `Refs #742`，不应使用 `Fixes #237` 或把完整 Layer 3 实现声明为已完成。`#743`-`#746` 的实现 PR 应同时引用 `#237` 与对应下游 issue，并声明其消费的 `#742` 输入对象。
...
- `Refs #237`，并同时引用对应下游 issue（`#743`、`#744`、`#745` 或 `#746`）
