...
本模型描述 `#237` / `#742` 在规约阶段必须冻结的共享对象与生命周期。`#742` 是完整 Layer 3 session rhythm 契约与状态机冻结输入；`#743`-`#746` 是后续实现与验证消费方。它在 `FR-0010/0011/#226` 的基础上追加 Layer 3 所需的阶段、窗口、事件、决策和状态视图模型，但不重定义既有审批、审计与风险状态对象。
