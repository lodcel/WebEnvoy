...
export type AbilityValidationMode = "smoke_validation" | "replay_validation";
export type AbilityValidationResultState = "verified" | "broken" | "stale";
export type AbilityHealthState = "unknown" | "healthy" | "degraded" | "broken" | "stale";
export type AbilityValidationCoverageState =
  | "none"
  | "smoke_only"
  | "replay_only"
  | "smoke_plus_replay"
  | "divergent";
export type AbilityFailureClass =
  | "page_changed"
  | "auth_or_session_required"
  | "gate_blocked"
  | "environment_mismatch"
  | "runtime_error";
export type AbilityTrustDomainKind = "read" | "download";
export type AbilityReplaySource = "last_success_input" | "explicit_input_snapshot";
...
export interface AbilityValidationRequest {
  ...
  expected_capability_kind: AbilityTrustDomainKind;
  smoke_input: JsonObject;
}

export type AbilityReplayRequest =
  | {
      ability_ref: string;
      profile_ref: string;
      requested_execution_layer: CandidateExecutionLayer;
...
export interface ReplayInputSnapshotRef {
  snapshot_ref: string;
  ability_ref: string;
  profile_ref: string;
  execution_layer: CandidateExecutionLayer;
...
export interface AbilityBaselineDescriptor {
  entrypoint: string;
...
export interface LatestValidationByMode {
  validation_mode: AbilityValidationMode;
  result_state: AbilityValidationResultState;
  failure_class?: AbilityFailureClass;
  validated_at: string;
  run_id: string;
  validated_execution_layer: CandidateExecutionLayer;
  baseline_descriptor: AbilityBaselineDescriptor;
  artifact_refs?: string[];
}

export interface AbilityHealthView {
  ability_ref: string;
  profile_ref: string;
  execution_layer: CandidateExecutionLayer;
  health_state: AbilityHealthState;
  validation_coverage_state: AbilityValidationCoverageState;
  latest_validations: LatestValidationByMode[];
  last_success_input_ref?: string;
  divergence_reason?: "smoke_replay_mismatch";
}

export interface BuildAbilityHealthViewInput {
  descriptor: CandidateAbilityDescriptor | unknown;
  profileRef: string;
  executionLayer: CandidateExecutionLayer;
  latestValidations?: LatestValidationByMode[];
  lastSuccessInputRef?: string | null;
  now?: Date;
}

const EXECUTION_LAYERS = new Set<CandidateExecutionLayer>(["L3", "L2", "L1"]);
const TRUST_DOMAIN_KINDS = new Set<AbilityTrustDomainKind>(["read", "download"]);
const VALIDATION_MODES = new Set<AbilityValidationMode>([
  "smoke_validation",
  "replay_validation"
]);
const RESULT_STATES = new Set<AbilityValidationResultState>([
  "verified",
  "broken",
  "stale"
]);
const FAILURE_CLASSES = new Set<AbilityFailureClass>([
  "page_changed",
  "auth_or_session_required",
  "gate_blocked",
  "environment_mismatch",
...
const invalidAbilityValidationInput = (reason: string, abilityId = "unknown"): CliError =>
  new CliError("ERR_CLI_INVALID_ARGS", "Ability validation input invalid", {
    details: {
...
const parseStringArray = (value: unknown, reason: string, abilityId: string): string[] | undefined => {
  if (value === undefined) {
    return undefined;
  }
...
const parseFailureClass = (
  value: unknown,
  reason: string,
  abilityId: string
): AbilityFailureClass => {
  if (typeof value !== "string" || !FAILURE_CLASSES.has(value as AbilityFailureClass)) {
    throw invalidAbilityValidationInput(reason, abilityId);
  }
  return value as AbilityFailureClass;
};

const parseTrustDomainKind = (
  value: unknown,
  reason: string,
  abilityId: string
): AbilityTrustDomainKind => {
  if (typeof value !== "string" || !TRUST_DOMAIN_KINDS.has(value as AbilityTrustDomainKind)) {
    throw invalidAbilityValidationInput(reason, abilityId);
  }
  return value as AbilityTrustDomainKind;
};

export const assertDescriptorIsInAbilityTrustDomain = (
  ...
  if (!TRUST_DOMAIN_KINDS.has(descriptor.ability_kind as AbilityTrustDomainKind)) {
    throw invalidAbilityValidationInput("ABILITY_KIND_NOT_IN_TRUST_DOMAIN", descriptor.ability_id);
  }
  return descriptor;
};

const assertExpectedKindMatchesDescriptor = (
  descriptor: CandidateAbilityDescriptor,
  expectedKind: AbilityTrustDomainKind
): void => {
...
const assertAbilityRefMatchesDescriptor = (
  descriptor: CandidateAbilityDescriptor,
  abilityRef: string
): void => {
...
export const parseAbilityValidationRequestForContract = (
  descriptorInput: CandidateAbilityDescriptor | unknown,
  value: unknown
): AbilityValidationRequest => {
  ...
  const abilityRef = parseRequiredString(
    object,
    "ability_ref",
    "ABILITY_REF_INVALID",
  ...
  const validationMode = parseRequiredString(
    object,
    "validation_mode",
  ...
  const profileRef = parseRequiredString(
    object,
    "profile_ref",
    "PROFILE_REF_INVALID",
    descriptor.ability_id
  ...
  const expectedCapabilityKind = parseTrustDomainKind(
    object.expected_capability_kind,
    "EXPECTED_CAPABILITY_KIND_INVALID",
    descriptor.ability_id
  );
...
export const parseAbilityReplayRequestForContract = (
  descriptorInput: CandidateAbilityDescriptor | unknown,
  value: unknown
): AbilityReplayRequest => {
  const descriptor = assertDescriptorIsInAbilityTrustDomain(descriptorInput);
  const object = asObject(value);
  if (!object) {
  ...
  assertExpectedKindMatchesDescriptor(descriptor, expectedCapabilityKind);
  const replaySource = parseRequiredString(
    object,
    "replay_source",
  ...
  const replayReason = parseRequiredString(
    object,
    "replay_reason",
  ...
  const replayInputRef = parseRequiredString(
    object,
    "replay_input_ref",
    "REPLAY_INPUT_REF_INVALID",
    descriptor.ability_id
  );
  return {
    ability_ref: abilityRef,
...
export const buildAbilityBaselineDescriptorForContract = (
  descriptorInput: CandidateAbilityDescriptor | unknown,
  profileRef: string
): AbilityBaselineDescriptor => {
  const descriptor = parseCandidateAbilityDescriptorForContract(descriptorInput);
...
export const buildReplayInputPayloadLocatorForContract = (snapshotRef: string): string => {
...
export const assertReplayInputSnapshotMatchesScopeForContract = (
  descriptorInput: CandidateAbilityDescriptor | unknown,
  snapshot: ReplayInputSnapshotRef | null | undefined,
  input: {
    profileRef: string;
    executionLayer: CandidateExecutionLayer;
  }
...
export const buildLatestValidationForContract = (input: {
  descriptor: CandidateAbilityDescriptor | unknown;
  profileRef: string;
  validationMode: AbilityValidationMode;
  resultState: "verified" | "broken";
  failureClass?: AbilityFailureClass | null;
  runId: string;
  validatedAt: string;
  validatedExecutionLayer: CandidateExecutionLayer;
  artifactRefs?: string[];
  ...
  return {
    ...
    baseline_descriptor: buildAbilityBaselineDescriptorForContract(
      descriptor,
      input.profileRef
    ),
...
const parseLatestValidation = (value: LatestValidationByMode, abilityId: string): LatestValidationByMode => {
  const object = asObject(value);
  if (!object) {
  ...
  if (!VALIDATION_MODES.has(validationMode as AbilityValidationMode)) {
  ...
  if (!RESULT_STATES.has(resultState as AbilityValidationResultState)) {
  ...
  const validatedAt = parseRequiredString(
    object,
    "validated_at",
  ...
  const validatedExecutionLayer = parseExecutionLayer(
    object.validated_execution_layer,
    "VALIDATED_EXECUTION_LAYER_INVALID",
    abilityId
  );
  ...
  const parsedSupport = executionLayerSupport.map((layer) =>
    parseExecutionLayer(layer, "BASELINE_EXECUTION_LAYER_SUPPORT_INVALID", abilityId)
  );
  const parsedBaseline: AbilityBaselineDescriptor = {
    entrypoint: parseRequiredString(baseline, "entrypoint", "BASELINE_ENTRYPOINT_INVALID", abilityId),
    input_contract_ref: parseRequiredString(
      baseline,
      "input_contract_ref",
      "BASELINE_INPUT_CONTRACT_REF_INVALID",
      abilityId
    ),
    output_contract_ref: parseRequiredString(
      baseline,
      "output_contract_ref",
      "BASELINE_OUTPUT_CONTRACT_REF_INVALID",
      abilityId
    ),
    error_contract_ref: parseRequiredString(
      baseline,
      "error_contract_ref",
      "BASELINE_ERROR_CONTRACT_REF_INVALID",
      abilityId
    ),
  ...
  return {
    validation_mode: validationMode as AbilityValidationMode,
    result_state: resultState as AbilityValidationResultState,
...
const latestIsStale = (input: {
  ...
  return (
    baseline.entrypoint !== input.descriptor.entrypoint ||
...
const normalizeLatestValidationForCurrentDescriptor = (input: {
  ...
  if (latest.validated_execution_layer !== input.executionLayer) {
    throw invalidAbilityValidationInput(
      "VALIDATED_EXECUTION_LAYER_SCOPE_MISMATCH",
      input.descriptor.ability_id
    );
  }
  if (!latestIsStale({
...
const modeRank: Record<AbilityValidationMode, number> = {
  smoke_validation: 0,
  replay_validation: 1
};

export const buildAbilityHealthViewForContract = (
  input: BuildAbilityHealthViewInput
): AbilityHealthView => {
  const descriptor = assertDescriptorIsInAbilityTrustDomain(input.descriptor);
  if (!EXECUTION_LAYERS.has(input.executionLayer)) {
  ...
  const byMode = new Map<AbilityValidationMode, LatestValidationByMode>();
  ...
  const latestValidations = [...byMode.values()].sort(
    (left, right) => modeRank[left.validation_mode] - modeRank[right.validation_mode]
  );
  ...
  let healthState: AbilityHealthState;
  ...
  let validationCoverageState: AbilityValidationCoverageState;
  if (currentLatest.length === 0) {
    validationCoverageState = "none";
  } else if (
    currentLatest.length === 1 &&
...
export const buildAbilityInvocationForValidationContract = (input: {
  descriptor: CandidateAbilityDescriptor | unknown;
  executionLayer: CandidateExecutionLayer;
  invocationInput: JsonObject;
}): ReturnType<typeof buildCandidateAbilityInvocationForContract> =>
  buildCandidateAbilityInvocationForContract(
    assertDescriptorIsInAbilityTrustDomain(input.descriptor),
    input.executionLayer,
    input.invocationInput
  );

export const classifyAbilityValidationFailureForContract = (error: unknown): AbilityFailureClass => {
...
export const parseAbilityFailureClassForContract = (
  value: unknown,
  abilityId = "unknown"
): AbilityFailureClass => parseFailureClass(value, "FAILURE_CLASS_INVALID", abilityId);

export const cloneReplayInputPayloadForContract = (value: unknown, abilityId = "unknown"): JsonObject => {
  const object = asObject(value);
  if (!object) {
...
export const cloneLatestValidationForContract = (
  latest: LatestValidationByMode
): LatestValidationByMode => ({
  ...latest,
  baseline_descriptor: {
