...
export interface LandBrowserDownloadArtifactInput {
  cwd: string;
  runId: string;
...
export interface LandedBrowserDownloadArtifact {
  resolvedOutputPath: string;
  trustedDownloadBase: string;
  fileName: string;
  sizeBytes: number;
...
const cliDownloadError = (
  reason: string,
  abilityId: string,
  stage: "input_validation" | "execution" | "output_mapping" = "execution"
): CliError =>
  new CliError(
    stage === "input_validation" ? "ERR_CLI_INVALID_ARGS" : "ERR_EXECUTION_FAILED",
    "Download artifact landing failed",
    {
      details: {
...
export const resolveTrustedDownloadBaseForContract = (cwd: string): string =>
  resolve(resolveRuntimeWorktreeRoot(cwd), ...TRUSTED_DOWNLOAD_SEGMENTS);
...
const assertActualDirectoryInsideTrustedBase = async (input: {
  ...
  abilityId: string;
}): Promise<{ realTrustedBase: string; realDestinationDir: string }> => {
...
const sanitizeFileName = (fileNameHint: string | undefined): string => {
...
const exists = async (fs: DownloadLandingFileSystem, path: string): Promise<boolean> => {
  try {
    await fs.stat(path);
    return true;
  } catch {
...
const resolveFinalPath = async (input: {
  ...
  destinationDir: string;
  fileName: string;
  conflictPolicy: DownloadAbilityRequest["output_policy"]["conflict_policy"];
  abilityId: string;
}): Promise<string> => {
...
const decodeBrowserArtifactContent = (
...
const buildArtifactRef = (runId: string, checksumSha256: string): string =>
  `download-artifact://${sanitizeToken(runId)}/${checksumSha256.slice(0, 16)}`;

export const landBrowserDownloadArtifactForContract = async (
  input: LandBrowserDownloadArtifactInput
): Promise<LandedBrowserDownloadArtifact> => {
  ...
  const destinationDir = resolveDestinationDirectory(
    trustedDownloadBase,
    input.request.output_policy.destination_root,
    input.request.ability_ref
  );
  ...
  const tempPath = resolve(
    realDestinationDir,
    `.${basename(finalPath)}.${sanitizeToken(input.runId)}.${process.pid}.tmp`
  );
