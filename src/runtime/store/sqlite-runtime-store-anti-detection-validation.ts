...
export class SQLiteRuntimeStoreAntiDetectionValidationRepository {
  ...
  readonly #notFound: (message: string) => RuntimeStoreError;

  constructor(input: AntiDetectionValidationRepositoryInput) {
    this.#db = input.db;
  ...
  async upsertAntiDetectionValidationRequest(
    ...
    try {
      ...
      if (existing) {
        ...
        this.#invalidInput(
          "anti-detection request_state terminal state requires an existing request"
        );
      }
