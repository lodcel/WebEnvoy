...
export interface XhsCreatorPublishAdmissionInputContract extends JsonObject {
...
export interface XhsControlledLiveWriteInputContract extends JsonObject {
  target_page: "creator_publish_tab";
...
export type XhsCommandInputContract =
  ...
  | XhsControlledLiveWriteInputContract
...
const XHS_COMMAND_ACTION_NAMES: Record<string, string> = {
  ...
  "xhs.creator_publish.controlled_live_write::xhs.creator.publish.v1":
    "xhs.controlled_live_upload_submit_publish",
...
const hasAllRequiredChecksTrue = (value: unknown): boolean => {
...
const resolveInferredIssueScopeForContract = (
  options: JsonObject
): "issue_208" | "issue_209" | "issue_753" | "issue_755" | "issue_835" | null => {
...
const resolveCanonicalIssueScopeForContract = (
  options: JsonObject
): "issue_209" | "issue_753" | "issue_755" | "issue_835" | null => {
  const explicitIssueScope = asString(options.issue_scope);
  if (
    explicitIssueScope === "issue_753" &&
    asString(options.action_type) === "write" &&
...
export const parseControlledLiveWriteInputForContract = (
  input: JsonObject = {},
  abilityId = "xhs.creator.publish.v1"
): XhsControlledLiveWriteInputContract => {
  const liveWriteAttemptId = parseRequiredStringFieldForContract(
    input,
    "live_write_attempt_id",
    "LIVE_WRITE_ATTEMPT_ID_INVALID",
    abilityId
  );
  const sourceMediaRef = parseRequiredStringFieldForContract(
    input,
    "source_media_ref",
    "SOURCE_MEDIA_REF_INVALID",
    abilityId
  );
  const sourceMediaKind = parseRequiredStringFieldForContract(
    input,
    "source_media_kind",
    "SOURCE_MEDIA_KIND_INVALID",
    abilityId
  );
  const sourceMediaDigest =
...
const isIssue835ControlledLiveWriteRequest = (options: JsonObject): options is JsonObject & {
  issue_scope: "issue_835";
  requested_execution_mode: "live_write";
} =>
  options.issue_scope === "issue_835" &&
  options.controlled_live_write === true &&
  options.action_type === "write" &&
...
const matchesIssue835Binding = (
  ...
  includeRiskState: boolean
): boolean =>
  asString(evidence.run_id) === input.runId &&
...
const resolveIssue835AdmissionDraftForContract = (input: {
  ...
  const canonicalApprovalEvidence: JsonObject = {
    ...approvalEvidence,
  ...
  const canonicalAuditEvidence: JsonObject = {
    ...auditEvidence,
    decision_id: decisionId,
    approval_id: approvalId,
    request_id: input.requestId,
...
const bindIssue835AdmissionDraftToSession = (
  draft: JsonObject,
  sessionId: string
): JsonObject | null => {
  ...
  const bindEvidence = (
    key: "approval_admission_evidence" | "audit_admission_evidence"
  ): void => {
...
const cloneIssue835AdmissionDraftForContract = (value: unknown): JsonObject | null => {
  const object = asObject(value);
  if (!object) {
    return null;
  }
...
export const prepareIssue835ControlledLiveWriteEnvelopeForContract = (input: {
  ...
  const commandRequestId =
    asString(input.requestId) ?? `${ISSUE835_LIVE_REQUEST_ID_PREFIX}-${randomUUID()}`;
  const gateInvocationId =
    asString(input.gateInvocationId) ??
...
export const bindIssue835ControlledLiveWriteEnvelopeToSessionForContract = (input: {
  ...
  const draft =
    cloneIssue835AdmissionDraftForContract(nextParams[ISSUE835_INTERNAL_ADMISSION_DRAFT_KEY]) ??
