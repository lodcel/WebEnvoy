...
const assertDedicatedXhsShorthandOptions = (options: JsonObject, ability: DedicatedXhsAbility): void => {
  ...
  if (unknownKeys.length > 0) {
    throw new CliError("ERR_CLI_INVALID_ARGS", "XHS dedicated command shorthand option invalid", {
      details: {
...
const normalizeDedicatedXhsCommandParams = (
  ...
  if (explicitAbility) {
    if (
      ...
      throw new CliError("ERR_CLI_INVALID_ARGS", "XHS dedicated command ability mismatch", {
        details: {
          ability_id: ability.id,
          stage: "input_validation",
