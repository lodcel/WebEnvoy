...
import {
  ...
  XHS_CLOSEOUT_TARGET_DOMAIN,
  XHS_CREATOR_LIVE_WRITE_ADMISSION_PROBE_BUNDLE_REF,
  XHS_CREATOR_PUBLISH_TARGET_PAGE,
...
type ExtensionSourceEquivalence = NonNullable<
...
const isAdmittedXhsValidationSourceScope = (input: {
  targetDomain: string | null;
  targetPage: string | null;
  requestedExecutionMode: string | null;
}): boolean =>
  (input.targetDomain === XHS_CLOSEOUT_TARGET_DOMAIN &&
...
const resolveXhsValidationSourceProbeBundleRef = (input: {
  ...
  throw new CliError(
    ...
    "XHS closeout validation source probe bundle scope invalid",
    {
      retryable: false,
      details: {
...
const sourcePageUrlMatchesSourceAudit = (pageUrl: string | null, sourceAudit: GateAuditRecord): boolean => {
  ...
  try {
    ...
    return (
      sourceAudit.target_page === XHS_CREATOR_PUBLISH_TARGET_PAGE &&
    ...
    asString(browserEvidence?.probe_bundle_ref) ===
      resolveXhsValidationSourceProbeBundleRef({
        ...
        requestedExecutionMode: input.sourceAudit
          .requested_execution_mode as AntiDetectionExecutionMode
      }) &&
...
const readXhsCloseoutValidationSignalsFromSourceSamples = async (input: {
  ...
  for (const [targetFrRef, validationScope] of requiredXhsCloseoutSampleScopes) {
    ...
    const expectedProbeBundleRef = resolveXhsValidationSourceProbeBundleRef({
      targetDomain: input.sourceAudit.target_domain,
      targetPage: input.sourceAudit.target_page,
      requestedExecutionMode: input.sourceAudit.requested_execution_mode as AntiDetectionExecutionMode
    });
...
const assertXhsCloseoutValidationSourceUrl = (input: {
  ...
  const normalizedPath = parsed.pathname.replace(/\/+$/u, "");
  const pageMatches =
    input.expectedPage === "search_result_tab"
  ...
  if (parsed.hostname !== input.expectedDomain || !pageMatches) {
    throw new CliError(
      "ERR_EXECUTION_FAILED",
      "XHS closeout validation source page does not match the admitted target",
      {
        retryable: false,
        details: {
          ability_id: "runtime.xhs_closeout_validation_source",
...
const runtimeXhsCloseoutValidationSource = async (context: RuntimeContext) => {
  ...
  if (
    ...
    throw new CliError(
      ...
      "XHS closeout validation source execution mode invalid",
      {
        details: {
          ability_id: "runtime.xhs_closeout_validation_source",
  ...
  if (
    !isAdmittedXhsValidationSourceScope({
      targetDomain,
      targetPage,
      requestedExecutionMode
    })
  ...
  const admittedTargetDomain = targetDomain as
    | typeof XHS_CLOSEOUT_TARGET_DOMAIN
    | typeof XHS_CREATOR_TARGET_DOMAIN;
  const admittedTargetPage = targetPage as
    | "search_result_tab"
    | typeof XHS_CREATOR_PUBLISH_TARGET_PAGE;
  const admittedRequestedExecutionMode = requestedExecutionMode as AntiDetectionExecutionMode;
  if (!targetTabId) {
...
const runtimeXhsCloseoutValidation = async (context: RuntimeContext) => {
  ...
  if (
    ...
    throw new CliError(
      "ERR_CLI_INVALID_ARGS",
      "XHS closeout validation execution mode invalid",
      {
        details: {
          ability_id: "runtime.xhs_closeout_validation",
          stage: "input_validation",
  ...
  if (
    ...
    throw new CliError(
      "ERR_CLI_INVALID_ARGS",
      "XHS closeout validation scope invalid",
      {
        details: {
          ability_id: "runtime.xhs_closeout_validation",
          stage: "input_validation",
...
const runtimeReloadTab = async (context: RuntimeContext) => {
  let bridge: NativeMessagingBridge | null = null;
  try {
    bridge = resolveRuntimeBridge();
    const result = await bridge.runCommand({
...
const runtimeXhsOpenResultCard = async (context: RuntimeContext) => {
  let bridge: NativeMessagingBridge | null = null;
  try {
    bridge = resolveRuntimeBridge();
    const result = await bridge.runCommand({
...
const runtimeXhsDebugResultTargets = async (context: RuntimeContext) => {
  let bridge: NativeMessagingBridge | null = null;
  try {
    bridge = resolveRuntimeBridge();
    const result = await bridge.runCommand({
...
const runtimeXhsDebugPageState = async (context: RuntimeContext) => {
  let bridge: NativeMessagingBridge | null = null;
  try {
    bridge = resolveRuntimeBridge();
    const result = await bridge.runCommand({
...
const isTransientAttachedRestorePendingStatus = (status: Record<string, unknown>): boolean =>
  status.lockHeld === true &&
...
const assertRuntimeRestoreXhsTargetSafetyGate = async (
  ...
  let persistedRhythmView: SessionRhythmStatusViewRecord | null = null;
  let store: SQLiteRuntimeStore | null = null;
  try {
    store = new SQLiteRuntimeStore(resolveRuntimeStorePath(context.cwd));
  ...
  const persistedRhythmBlocksRestore =
    persistedRhythmPhase === "cooldown" || persistedRhythmDecision === "blocked";
  const rhythmAllowsRestore =
    !persistedRhythmBlocksRestore &&
  ...
  throw new CliError("ERR_EXECUTION_FAILED", "XHS target restoration gate blocked current request", {
    ...
    details: {
      ...
      ...(persistedRhythmView
        ? {
            session_rhythm_status_view: buildPersistedSessionRhythmStatusView(
              persistedRhythmView
            )
          }
        : {}),
